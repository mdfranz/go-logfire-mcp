...
**MCP bootstrap (`cmd/logfire-mcp/main.go`)** — `run() int` pattern, logger to stderr by default (an explicit path enables 10 MiB rotation; `"off"` disables logging), opt-in PID lockfile (default `"off"`; explicit path enables it), `signal.NotifyContext` for graceful shutdown, `mcp.NewServer`, and `s.Run(ctx, &mcp.StdioTransport{})`. Log every resolved non-secret config value at startup, including whether locking is disabled, but never the token. **Invariant: nothing writes to stdout except the MCP stdio transport itself.** Independent stdio server processes must work concurrently when locking is off; users who direct multiple instances to the same rotating logfile are also expected to enable the shared lock or choose distinct paths.
