...
- **Auth**: `LOGFIRE_API_KEY`, required and preferred for current scoped access. It must include `project:read_oltp`; `LOGFIRE_READ_TOKEN` and `LOGFIRE_API_TOKEN` remain compatibility aliases for existing project read tokens or API keys.
...
2. CLI smoke test against the real API (requires `LOGFIRE_API_KEY` with `project:read_oltp`, or a legacy project read token):
...
4. MCP server manual run: `LOGFIRE_MCP_LOGFILE=off LOGFIRE_API_KEY=... go run ./cmd/logfire-mcp` — locking is off by default; confirm zero stdout bytes before a client connects and clean shutdown on Ctrl+C.
